    ...
    stage('Prepare') {
      steps {
        checkout([$class: 'GitSCM',
          ...
          sh 'sh \'curl -LO https://storage.googleapis.com/kubernetes-release/release/$(curl -s https://storage.googleapis.com/kubernetes-release/release/stable.txt)/bin/linux/amd64/kubectl\''
        }
      }
      stage('Build') {
        when {
